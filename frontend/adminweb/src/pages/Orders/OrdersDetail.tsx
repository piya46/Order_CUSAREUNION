...
// ✅ 1. ย้ายมาประกาศข้างบนสุด (Global Scope ของไฟล์นี้) และใส่ URL จริงกันเหนียว
...
export default function OrdersDetail() {
  ...
  const fetchSlipUrl = async (orderId: string) => {
    ...
    try {
      ...
      if (rawUrl) {
        // 1. แกะ Path (ระวังเรื่อง Relative/Absolute URL)
        const urlObj = new URL(rawUrl, window.location.origin);
        const pathAndQuery = urlObj.pathname + urlObj.search;

        // 2. สร้าง URL ใหม่ โดยใช้ Domain ที่เรา Hardcode ไว้ข้างบน
        // ผลลัพธ์จะเป็น https://api.cusa... + /api/files/...
        const finalUrl = `${TARGET_API_ORIGIN}${pathAndQuery}`;
        
        console.log("✅ New Logic Applied. Fixed URL:", finalUrl); 
        setSlipUrl(finalUrl);
      }
    } catch (error) {
      console.error("Error fetching slip URL:", error);
  ...
  const onDelete = async () => {
      if(!confirm("⚠️ ยืนยันการลบออเดอร์นี้ถาวร?")) return;
      try {
          await fetch(`${TARGET_API_ORIGIN}/api/orders/${id}`, { 
             method: 'DELETE', 
             headers: { Authorization: `Bearer ${localStorage.getItem("aw_token")}` } 
          });
          nav("/orders");
